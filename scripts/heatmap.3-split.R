...
#           if (!is.na(key.ylab))
#             par(mgp=c(2.8, 1, 0))
#             mtext(side=2,key.ylab, line=par("mgp")[1], padj=0.5,cex = .8)
#         }
#       else
        title("Color Key")
...
for(i in 1:nrow(avgA)){
  ...
  for (i in 1:classCount) { 
    ...
    # add a legend 
    #   legend(18, .9, 1:classCount, xjust=1,yjust=1,cex=0.8, col=colors,
    #          pch=plotchar, lty=linetype, title="Classes")
